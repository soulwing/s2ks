...
After successfully running the demo, if you look in the S3 bucket you should see
...
the `DEK-Info` header provides additional non-secret details such as the 
initialization vector (IV) needed to initialize the cipher when decrypting 
the key.
...
discards from memory. The KMS also returns an encrypted copy of the data 
...
the blob from the first key in the file back to the KMS and asks the KMS to 
